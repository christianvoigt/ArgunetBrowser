...
this._setOption("step",this.options.step);this._value(this.element.val(),true);this._draw();this._on(this._events);this._refresh();this._on(this.window,{beforeunload:function(){this.element.removeAttr("autocomplete")}})},_getCreateOptions:function(){var c={},d=this.element;b.each(["min","max","step"],function(e,f){var g=d.attr(f);if(g!==undefined&&g.length){c[f]=g}});return c},_events:{keydown:function(c){if(this._start(c)&&this._keydown(c)){c.preventDefault()}},keyup:"_stop",focus:function(){this.previous=this.element.val()},blur:function(c){if(this.cancelBlur){delete this.cancelBlur;return}this._refresh();if(this.previous!==this.element.val()){this._trigger("change",c)}},mousewheel:function(c,d){if(!d){return}if(!this.spinning&&!this._start(c)){return false}this._spin((d>0?1:-1)*this.options.step,c);clearTimeout(this.mousewheelTimer);this.mousewheelTimer=this._delay(function(){if(this.spinning){this._stop(c)}},100);c.preventDefault()},"mousedown .ui-spinner-button":function(d){var c;c=this.element[0]===this.document[0].activeElement?this.previous:this.element.val();function e(){var f=this.element[0]===this.document[0].activeElement;if(!f){this.element.focus();this.previous=c;this._delay(function(){this.previous=c})}}d.preventDefault();e.call(this);this.cancelBlur=true;this._delay(function(){delete this.cancelBlur;e.call(this)});if(this._start(d)===false){return}this._repeat(null,b(d.currentTarget).hasClass("ui-spinner-up")?1:-1,d)},"mouseup .ui-spinner-button":"_stop","mouseenter .ui-spinner-button":function(c){if(!b(c.currentTarget).hasClass("ui-state-active")){return}if(this._start(c)===false){return false}this._repeat(null,b(c.currentTarget).hasClass("ui-spinner-up")?1:-1,c)},"mouseleave .ui-spinner-button":"_stop"},_draw:function(){var c=this.uiSpinner=this.element.addClass("ui-spinner-input").attr("autocomplete","off").wrap(this._uiSpinnerHtml()).parent().append(this._buttonHtml());this.element.attr("role","spinbutton");this.buttons=c.find(".ui-spinner-button").attr("tabIndex",-1).button().removeClass("ui-corner-all");if(this.buttons.height()>Math.ceil(c.height()*0.5)&&c.height()>0){c.height(c.height())}if(this.options.disabled){this.disable()}},_keydown:function(d){var c=this.options,e=b.ui.keyCode;switch(d.keyCode){case e.UP:this._repeat(null,1,d);return true;case e.DOWN:this._repeat(null,-1,d);return true;case e.PAGE_UP:this._repeat(null,c.page,d);return true;case e.PAGE_DOWN:this._repeat(null,-c.page,d);return true}return false},_uiSpinnerHtml:function(){return"<span class='ui-spinner ui-widget ui-widget-content ui-corner-all'></span>"},_buttonHtml:function(){return"<a class='ui-spinner-button ui-spinner-up ui-corner-tr'><span class='ui-icon "+this.options.icons.up+"'>&#9650;</span></a><a class='ui-spinner-button ui-spinner-down ui-corner-br'><span class='ui-icon "+this.options.icons.down+"'>&#9660;</span></a>"},_start:function(c){if(!this.spinning&&this._trigger("start",c)===false){return false}if(!this.counter){this.counter=1}this.spinning=true;return true},_repeat:function(d,c,e){d=d||500;clearTimeout(this.timer);this.timer=this._delay(function(){this._repeat(40,c,e)},d);this._spin(c*this.options.step,e)},_spin:function(d,c){var e=this.value()||0;if(!this.counter){this.counter=1}e=this._adjustValue(e+d*this._increment(this.counter));if(!this.spinning||this._trigger("spin",c,{value:e})!==false){this._value(e);this.counter++}},_increment:function(c){var d=this.options.incremental;if(d){return b.isFunction(d)?d(c):Math.floor(c*c*c/50000-c*c/500+17*c/200+1)}return 1},_precision:function(){var c=this._precisionOf(this.options.step);if(this.options.min!==null){c=Math.max(c,this._precisionOf(this.options.min))}return c},_precisionOf:function(d){var e=d.toString(),c=e.indexOf(".");return c===-1?0:e.length-c-1},_adjustValue:function(e){var d,f,c=this.options;d=c.min!==null?c.min:0;f=e-d;f=Math.round(f/c.step)*c.step;e=d+f;e=parseFloat(e.toFixed(this._precision()));if(c.max!==null&&e>c.max){return c.max}if(c.min!==null&&e<c.min){return c.min
}return e},_stop:function(c){if(!this.spinning){return}clearTimeout(this.timer);clearTimeout(this.mousewheelTimer);this.counter=0;this.spinning=false;this._trigger("stop",c)},_setOption:function(c,d){if(c==="culture"||c==="numberFormat"){var e=this._parse(this.element.val());this.options[c]=d;this.element.val(this._format(e));return}if(c==="max"||c==="min"||c==="step"){if(typeof d==="string"){d=this._parse(d)}}if(c==="icons"){this.buttons.first().find(".ui-icon").removeClass(this.options.icons.up).addClass(d.up);this.buttons.last().find(".ui-icon").removeClass(this.options.icons.down).addClass(d.down)}this._super(c,d);if(c==="disabled"){if(d){this.element.prop("disabled",true);this.buttons.button("disable")}else{this.element.prop("disabled",false);this.buttons.button("enable")}}},_setOptions:a(function(c){this._super(c);this._value(this.element.val())}),_parse:function(c){if(typeof c==="string"&&c!==""){c=window.Globalize&&this.options.numberFormat?Globalize.parseFloat(c,10,this.options.culture):+c}return c===""||isNaN(c)?null:c},_format:function(c){if(c===""){return""}return window.Globalize&&this.options.numberFormat?Globalize.format(c,this.options.numberFormat,this.options.culture):c},_refresh:function(){this.element.attr({"aria-valuemin":this.options.min,"aria-valuemax":this.options.max,"aria-valuenow":this._parse(this.element.val())})},_value:function(e,c){var d;if(e!==""){d=this._parse(e);if(d!==null){if(!c){d=this._adjustValue(d)}e=this._format(d)}}this.element.val(e);this._refresh()},_destroy:function(){this.element.removeClass("ui-spinner-input").prop("disabled",false).removeAttr("autocomplete").removeAttr("role").removeAttr("aria-valuemin").removeAttr("aria-valuemax").removeAttr("aria-valuenow");this.uiSpinner.replaceWith(this.element)},stepUp:a(function(c){this._stepUp(c)}),_stepUp:function(c){if(this._start()){this._spin((c||1)*this.options.step);this._stop()}},stepDown:a(function(c){this._stepDown(c)}),_stepDown:function(c){if(this._start()){this._spin((c||1)*-this.options.step);this._stop()}},pageUp:a(function(c){this._stepUp((c||1)*this.options.page)}),pageDown:a(function(c){this._stepDown((c||1)*this.options.page)}),value:function(c){if(!arguments.length){return this._parse(this.element.val())}a(this._value).call(this,c)},widget:function(){return this.uiSpinner}})}(jQuery));(jQuery.effects||(function(a,c){var b="ui-effects-";a.effects={effect:{}};
/*!
...
},oval:function(r,v,s,u,t){t=t||{};f.prototype._draw(r,v,s,u,t)},line:function(s,v,r,t,u){var w=new a(s,v,r,t);w.draw(u)},lines:function(s,u,r,t){if(typeof t=="number"){k.push([{x:s,y:u},{x:r,y:t}])}else{k.push([s,u])}},drawLines:function(r){var s=new a(k);s.draw(r);k=[]}};return j.init()};arbor=(typeof(arbor)!=="undefined")?arbor:{};$.extend(arbor,{Graphics:function(ctx){return Graphics(ctx)},colors:{CSS:Colors.CSS,validate:Colors.validate,decode:Colors.decode,encode:Colors.encode,blend:Colors.blend}})})(this.jQuery);this.argunet=this.argunet||{};argunet.Argument=function(c,e,b,f,d,a){this.id=c;this.title=e;this.description=b;this.sentences=[];this.colorIndex=f;this.group=d;this.relations=a};argunet.Argument.prototype.addSentence=function(d,b,a){var c=new Sentence(d,b,a);this.sentences[b]=c};this.argunet=this.argunet||{};(function(){var b=function(){this.nodes={};this.colors=[];this.groups={};this.edgesBySource={};this.edgesByTarget={}};var a=b.prototype;a.loadDebate=function(d){var e=this;var c=$(d).find("arg\\:debate, debate").get(0);this.debateTitle=$(c).children("arg\\:title").text();this.debateSubtitle=$(c).children("arg\\:subtitle").text();$(d).find("arg\\:color, color").each(function(){var j=$(this).attr("colorIndex");var h=$(this).attr("description");var i=$(this).attr("colorCode");var g=$.xcolor.lighten(i,80,1).getHex();var f=$.xcolor.darken(i,80,2).getHex();e.colors[j]={index:j,description:h,base:i,light:g,dark:f}});$(d).find("node").each(function(){if($(this).children("data:first").children("arg\\:group").length){var h=$(this).children("data:first").children("arg\\:group");var i=$(h).children("arg\\:title").text();var u=$(h).attr("numberOfArguments");var f=$(h).attr("numberOfTheses");var o=$(h).attr("color");var n=$(h).attr("id");var t=new argunet.Group(n,i,o,u,f);e.groups[n]=t}else{if($(this).children("data:first").children("arg\\:argument").length){var j=$(this).children("data:first").children("arg\\:argument:first");var v=$(j).attr("id");var q=$(j).children("arg\\:title").text();var s=$(j).children("arg\\:description").text();var r=$(j).attr("colorIndex");var p={incomingAttacks:$(j).attr("incomingAttacks"),incomingSupports:$(j).attr("incomingSupports"),outgoingAttacks:$(j).attr("outgoingAttacks"),outgoingSupports:$(j).attr("outgoingSupports")};var t;if($(this).parents("node:first").length){var n=$(this).parents("node:first").children("data:first").children("arg\\:group").attr("id");e.groups[n].addNode(v);t=n}var g=new argunet.Argument(v,q,s,r,t,p);e.nodes[v]=g}else{if($(this).children("data:first").children("arg\\:thesis").length){var l=$(this).children("data:first").children("arg\\:thesis:first");var v=$(l).attr("id");var q=$(l).children("arg\\:title").text();var k=$(l).children("arg\\:content").text();var r=$(l).attr("colorIndex");var t;var p={incomingAttacks:$(l).attr("incomingAttacks"),incomingSupports:$(l).attr("incomingSupports"),outgoingAttacks:$(l).attr("outgoingAttacks"),outgoingSupports:$(l).attr("outgoingSupports")};if($(this).parents("node:first").length){var n=$(this).parents("node:first").children("data:first").children("arg\\:group").attr("id");e.groups[n].addNode(v);t=n}var m=new argunet.Thesis(v,q,k,r,t,p);e.nodes[v]=m}}}});$(d).find("edge:parent").each(function(){var g=$(this).children("data:first").children("arg\\:edge");var f=g.attr("type");edge=new argunet.Edge($(g).attr("sourceNodeId"),$(g).attr("targetNodeId"),f,"#00CC00",$(g).attr("sourcePropositionId"),$(g).attr("targetPropositionId"));if(e.edgesBySource[edge.source]==undefined){e.edgesBySource[edge.source]={}}if(e.edgesByTarget[edge.target]==undefined){e.edgesByTarget[edge.target]={}}e.edgesBySource[edge.source][edge.target]=edge;e.edgesByTarget[edge.target][edge.source]=edge})};a.getNode=function(c){return this.nodes[c]};a.getColor=function(c){return this.colors[c]};a.getEdge=function(d,c){return this.edgesBySource[d][c]};a.getEdgesFrom=function(c){return this.edgesBySource[c]};a.getEdgesTo=function(c){return this.edgesByTarget[c]};argunet.DebateManager=b}());this.argunet=this.argunet||{};argunet.Edge=function(f,d,e,c,a,b){this.source=f;this.target=d;this.sourceProposition=a;this.targetProposition=b;this.type=e;this.color=c};this.argunet=this.argunet||{};argunet.Group=function(e,d,a,b,c){this.id=e;this.title=d;this.color=a;this.numberOfArguments=parseInt(b);this.numberOfTheses=parseInt(c);this.nodes=[];this.open=true};argunet.Group.prototype.addNode=function(a){this.nodes.push(a)};this.argunet=this.argunet||{};argunet.HistoryState=function(a,b){this.type=a;this.data=b};this.argunet=this.argunet||{};argunet.Sentence=function(c,b,a){if(a!="conclusion"){a="premise"}this.text=c;this.nr=b;this.type=a};this.argunet=this.argunet||{};argunet.Thesis=function(c,e,b,f,d,a){this.id=c;this.argunetId=c;this.title=e;this.content=b;this.colorIndex=f;this.group=d;this.relations=a};this.argunet=this.argunet||{};(function(){var a=function(e,f){var i=argunet.ArborController.prototype;i.addEventListener=null;i.removeEventListener=null;i.removeAllEventListeners=null;i.dispatchEvent=null;i.hasEventListener=null;i._listeners=null;createjs.EventDispatcher.initialize(i);this.debateManager=f;var h=arbor.ParticleSystem(400,400,0.7,true,55,0.03);h.parameters({integrator:"euler"});var e=e;e.addEventListener("mousedown",this);h.renderer=e;this.graphDepth=1;var d={};var c={};var g=undefined;this.addNodeToVisibleGraph=function(o,j){var l=(j==0)?true:false;var m=this;var n=this.debateManager.groups[o];var k;if(!n){k=this.debateManager.nodes[o].group}else{k=n.id}if(n||(k!=undefined&&!this.debateManager.groups[k].open)){if(d[k]!=undefined){d[k].selected=l;return}d[k]={id:k,group:true,selected:l,edgesToSelectedNode:j}}else{if(d[o]!=undefined){d[o].selected=l;return}d[o]={id:o,selected:l,edgesToSelectedNode:j}}};this.addEdgeToVisibleGraph=function(m){var l=this.debateManager.nodes[m.source].group;var p=this.debateManager.nodes[m.target].group;var r=this.debateManager.groups[l];var o=this.debateManager.groups[p];var k=m.source;var q=m.target;if(r!=undefined&&!r.open){k=l}if(o!=undefined&&!o.open){q=p}if(c[k]!=undefined&&c[k][q]!=undefined){return}if(c[k]==undefined){c[k]={}}var j=0;var n=0;c[k][q]={source:k,target:q,sourceModel:m.source,targetModel:m.target}};this.changeGraphDepth=function(k,l){var j=this;if(k<l){$.each(d,function(n,p){var o=p;if(o.group){if(o.edgesToSelectedNode==k){var q=j.debateManager.groups[o.id];var m=q.nodes;$.each(m,function(r,t){var s=t;if(j.debateManager.getEdgesTo(s)!=undefined){$.each(j.debateManager.getEdgesTo(s),function(u,v){j.addNodeToVisibleGraph(v.source,k+1);j.addEdgeToVisibleGraph(v)})}if(j.debateManager.getEdgesFrom(s)!=undefined){$.each(j.debateManager.getEdgesFrom(s),function(u,v){j.addNodeToVisibleGraph(v.target,k+1);j.addEdgeToVisibleGraph(v)})}});if(q.open&&g==q.id){g=undefined;delete d[q.id]}j.changeGraphDepth(k+1,l)}}else{if(o.edgesToSelectedNode==k){if(j.debateManager.getEdgesTo(o.id)!=undefined){$.each(j.debateManager.getEdgesTo(o.id),function(r,s){j.addNodeToVisibleGraph(s.source,k+1);j.addEdgeToVisibleGraph(s)})}if(j.debateManager.getEdgesFrom(o.id)!=undefined){$.each(j.debateManager.getEdgesFrom(o.id),function(r,s){j.addNodeToVisibleGraph(s.target,k+1);j.addEdgeToVisibleGraph(s)})}j.changeGraphDepth(k+1,l)}}})}else{return}};this.setGraphDepth=function(j){this.graphDepth=j;this.update()};this.selectNode=function(j){if(g==j){return}g=j;this.update()};this.update=function(){d={};c={};this.addNodeToVisibleGraph(g,0);this.changeGraphDepth(0,this.graphDepth);h.merge({nodes:d,edges:c})}};var b=a.prototype;b.handleEvent=function(c){if(c.type=="historyChange"){var d=c.state;if(d.type=="nodeSelection"){this.selectNode(d.data.nodeId)}}else{if(c.type=="graphDepthChange"){this.graphDepth=c.value;this.update()}else{if(c.type=="openGroup"){this.debateManager.groups[c.id].open=true;this.update()}else{if(c.type=="closeGroup"){this.debateManager.groups[c.id].open=false;this.update()}else{if(c.type=="openAllGroups"){$.each(this.debateManager.groups,function(){this.open=true});this.update()}else{if(c.type=="closeAllGroups"){$.each(this.debateManager.groups,function(){this.open=false});this.update()}}}}}}};argunet.ArborController=a}());this.argunet=this.argunet||{};argunet.BrowserRegistry=function(){if(arguments.callee.instance){return arguments.callee.instance}arguments.callee.instance=this;this.browsers=[];this.registerBrowser=function(a){this.browsers.push(a);return"argunetBrowser-"+this.browsers.length}};argunet.BrowserRegistry.getInstance=function(){var a=new argunet.BrowserRegistry();return a};this.argunet=this.argunet||{};(function(){var a=function(c,d){this.view=c;this.manager=d;var e=this;var b={};var g=[];var f=function(l,i){var h=e.manager.getNode(l);var k="";if(h instanceof argunet.Argument){k="Argument: "+h.title}else{if(h instanceof argunet.Thesis){k="Thesis: "+h.title}}var j={name:k,nodeId:h.id,nocheck:true};if(i!=undefined){i.children.push(j);b[l]=true}else{g.push(j)}};$.each(this.manager.groups,function(){var h={id:this.id,name:"Hide in Group: "+this.title,children:[],checked:false};$.each(this.nodes,function(){f(this,h)});g.push(h)});$.each(this.manager.nodes,function(){if(!b[this.id]){f(this.id)}});this.view.initialize(this.manager.debateTitle,this.manager.debateSubtitle,g)};argunet.DebateListController=a}());this.argunet=this.argunet||{};argunet.History=function(){var b=argunet.History.prototype;b.addEventListener=null;b.removeEventListener=null;b.removeAllEventListeners=null;b.dispatchEvent=null;b.hasEventListener=null;b._listeners=null;createjs.EventDispatcher.initialize(b);var c=-1;var a=[];this.backwardEnabled=false;this.forwardEnabled=false;this.homeEnabled=false;this.addState=function(d){if(!d instanceof argunet.HistoryState){throw new Error("'state' is not an instance of argunet.HistoryState");return}c++;if(c<a.length-1){a.splice(c,a.length-c-1,d);this.forwardEnabled=false}else{a[c]=d}if(a.length>1){this.backwardEnabled=true}if(c==0){this.homeEnabled=false}else{this.homeEnabled=true}this.dispatchEvent({type:"historyChange",state:this.getCurrentState()},this)};this.back=function(){if(!this.backwardEnabled){return}if(c>0){c--;this.forwardEnabled=true}if(c==0){this.backwardEnabled=false;this.homeEnabled=false}else{this.homeEnabled=true}this.dispatchEvent({type:"historyChange",state:this.getCurrentState()},this)};this.forward=function(){if(!this.forwardEnabled){return}if(c<a.length-1){c++;this.backwardEnabled=true;this.homeEnabled=true}if(c==a.length-1){this.forwardEnabled=false}this.dispatchEvent({type:"historyChange",state:this.getCurrentState()},this)};this.home=function(){if(!this.homeEnabled){return}c=0;this.backwardEnabled=false;this.homeEnabled=false;if(a.length>0){this.forwardEnabled=true}this.dispatchEvent({type:"historyChange",state:this.getCurrentState()},this)};this.getCurrentState=function(){return a[c]};this.selectNode=function(d){if(this.selectedNode!=undefined&&this.selectedNode==d){return}this.selectedNode=d;this.addState(new argunet.HistoryState("nodeSelection",{nodeId:d}))};this.handleEvent=function(d){if(d.type=="back"){this.back()}else{if(d.type=="home"){this.home()}else{if(d.type=="forward"){this.forward()}}}}};this.argunet=this.argunet||{};(function(){var a=function(){};var b=a.prototype=new createjs.Container();b.Container_initialize=a.prototype.initialize;b.Container_tick=a.prototype._tick;b.initialize=function(g,f,e,c){this.Container_initialize();this.nodeId=g;this.colors=c;this.title=f;this.text=e;this.state0Height=15;this.cornerRadius=8;this.styleState1={width:100,bgColor:this.colors.base,borderColor:this.colors.dark,borderWidth:1,cornerRadius:8,titleColor:this.colors.dark,titleCss:"11px Arial",textColor:this.colors.dark};this.styleState2={width:130,bgColor:this.colors.base,borderColor:this.colors.dark,borderWidth:3,cornerRadius:8,titleColor:this.colors.dark,titleCss:"bold 11px Arial",textColor:this.colors.dark,textCss:"11px Arial"};this.styleState0={cornerRadius:8};this.paddingState1={top:4,left:4,bottom:4,right:4};this.paddingState2={top:8,left:8,bottom:8,right:8};this.marginState1={top:1,right:1,bottom:1,left:1};this.marginState2={top:2,right:2,bottom:2,left:2};var d=this;this.addEventListener("mousedown",this)};b.handleEvent=function(c){if(c.type=="mousedown"){this.lastMousedown=new Date().getTime();var d=this;c.onMouseMove=function(e){e.target.x=e.stageX;e.target.y=e.stageY;d.dispatchEvent("drag",d)};c.onMouseUp=function(e){var f=new Date().getTime();if(f-d.lastMousedown<200){d.dispatchEvent("select",d)}d.dispatchEvent("drop",d)}}};b.render=function(){this.state0=new createjs.Container();this.state1=new createjs.Container();this.state2=new createjs.Container();this.addChild(this.state0);this.addChild(this.state1);this.addChild(this.state2);var f=new createjs.Graphics();f.beginFill(this.styleState1.bgColor);f.setStrokeStyle(this.styleState1.borderWidth).beginStroke(this.styleState1.borderColor);f.drawRoundRect(0,0,this.state0Height,this.state0Height,this.styleState0.cornerRadius);var e=new createjs.Shape(f);e.x=-this.state0Height/2;e.y=-this.state0Height/2;this.state0.addChild(e);var c=new createjs.Text(this.title,this.styleState1.titleCss,this.styleState1.titleColor);c.lineWidth=this.styleState1.width-this.paddingState1.left-this.paddingState1.right;c.textAlign="center";this.state1Height=c.getMeasuredHeight()+this.paddingState1.top+this.paddingState1.bottom;var f=new createjs.Graphics();f.beginFill(this.styleState1.bgColor);f.setStrokeStyle(this.styleState1.borderWidth).beginStroke(this.styleState1.borderColor);f.drawRoundRect(0,0,this.styleState1.width,this.state1Height,this.styleState2.cornerRadius);var d=new createjs.Shape(f);d.x=-this.styleState1.width/2;d.y=-this.state1Height/2;this.state1.addChild(d);c.x=d.x+this.styleState1.width/2;c.y=d.y+this.paddingState1.top;this.state1.addChild(c);this.renderState2();this.deselect()};b.renderState2=function(){var h=new createjs.Text(this.title,this.styleState2.titleCss,this.styleState2.titleColor);h.lineWidth=this.styleState2.width-this.paddingState2.left-this.paddingState2.right;h.textAlign="center";var c=200;var e=(this.text.length>c)?this.text.substring(0,this.text.length-(this.text.length-c))+" ...":this.text;var g=new createjs.Text(e,this.styleState2.textCss,this.styleState2.textColor);g.lineWidth=this.styleState2.width-this.paddingState2.left-this.paddingState2.right;g.textAlign="center";this.state2Height=this.paddingState2.top+h.getMeasuredHeight()+this.paddingState2.top/2+g.getMeasuredHeight()+this.paddingState2.bottom;var f=new createjs.Graphics();f.beginFill(this.styleState2.bgColor);f.setStrokeStyle(this.styleState2.borderWidth).beginStroke(this.styleState2.borderColor);f.drawRoundRect(0,0,this.styleState2.width,this.state2Height,this.styleState2.cornerRadius);var d=new createjs.Shape(f);d.x=-this.styleState2.width/2;d.y=-this.state2Height/2;h.x=g.x=d.x+this.styleState2.width/2;h.y=d.y+this.paddingState2.top;g.y=h.y+h.getMeasuredHeight()+this.paddingState2.top/2;this.state2.addChild(d);this.state2.addChild(h);this.state2.addChild(g)};b.getMarginedHeight=function(){if(this.state1.visible){return this.state1Height+this.marginState1.top+this.marginState1.bottom}else{if(this.state2.visible){return this.state2Height+this.marginState2.top+this.marginState2.bottom
}else{return this.state0Height}}};b.getMarginedWidth=function(){if(this.state1.visible){return this.styleState1.width+this.marginState1.top+this.marginState1.bottom}else{if(this.state2.visible){return this.styleState2.width+this.marginState1.left+this.marginState1.right}else{return this.state0Height}}};b.select=function(){this.state0.visible=false;this.state1.visible=false;this.state2.visible=true};b.deselect=function(){this.state0.visible=false;this.state1.visible=true;this.state2.visible=false};b.minimize=function(){this.state0.visible=true;this.state1.visible=false;this.state2.visible=false};b.render=function(){this.state0=new createjs.Container();this.state1=new createjs.Container();this.state2=new createjs.Container();this.addChild(this.state0);this.addChild(this.state1);this.addChild(this.state2);var f=new createjs.Graphics();f.beginFill(this.styleState1.bgColor);f.setStrokeStyle(this.styleState1.borderWidth).beginStroke(this.styleState1.borderColor);f.drawRoundRect(0,0,this.state0Height,this.state0Height,this.styleState0.cornerRadius);var e=new createjs.Shape(f);e.x=-this.state0Height/2;e.y=-this.state0Height/2;this.state0.addChild(e);var c=new createjs.Text(this.title,this.styleState1.titleCss,this.styleState1.titleColor);c.lineWidth=this.styleState1.width-this.paddingState1.left-this.paddingState1.right;c.textAlign="center";this.state1Height=c.getMeasuredHeight()+this.paddingState1.top+this.paddingState1.bottom;var f=new createjs.Graphics();f.beginFill(this.styleState1.bgColor);f.setStrokeStyle(this.styleState1.borderWidth).beginStroke(this.styleState1.borderColor);f.drawRoundRect(0,0,this.styleState1.width,this.state1Height,this.styleState2.cornerRadius);var d=new createjs.Shape(f);d.x=-this.styleState1.width/2;d.y=-this.state1Height/2;this.state1.addChild(d);c.x=d.x+this.styleState1.width/2;c.y=d.y+this.paddingState1.top;this.state1.addChild(c);this.renderState2();this.deselect()};b.renderState2=function(){var h=new createjs.Text(this.title,this.styleState2.titleCss,this.styleState2.titleColor);h.lineWidth=this.styleState2.width-this.paddingState2.left-this.paddingState2.right;h.textAlign="center";var c=200;var e=(this.text.length>c)?this.text.substring(0,this.text.length-(this.text.length-c))+" ...":this.text;var g=new createjs.Text(e,this.styleState2.textCss,this.styleState2.textColor);g.lineWidth=this.styleState2.width-this.paddingState2.left-this.paddingState2.right;g.textAlign="center";this.state2Height=this.paddingState2.top+h.getMeasuredHeight()+this.paddingState2.top/2+g.getMeasuredHeight()+this.paddingState2.bottom;var f=new createjs.Graphics();f.beginFill(this.styleState2.bgColor);f.setStrokeStyle(this.styleState2.borderWidth).beginStroke(this.styleState2.borderColor);f.drawRoundRect(0,0,this.styleState2.width,this.state2Height,this.styleState2.cornerRadius);var d=new createjs.Shape(f);d.x=-this.styleState2.width/2;d.y=-this.state2Height/2;h.x=g.x=d.x+this.styleState2.width/2;h.y=d.y+this.paddingState2.top;g.y=h.y+h.getMeasuredHeight()+this.paddingState2.top/2;this.state2.addChild(d);this.state2.addChild(h);this.state2.addChild(g)};b.getMarginedHeight=function(){if(this.state1.visible){return this.state1Height+this.marginState1.top+this.marginState1.bottom}else{if(this.state2.visible){return this.state2Height+this.marginState2.top+this.marginState2.bottom}else{return this.state0Height}}};b.getMarginedWidth=function(){if(this.state1.visible){return this.styleState1.width+this.marginState1.top+this.marginState1.bottom}else{if(this.state2.visible){return this.styleState2.width+this.marginState1.left+this.marginState1.right}else{return this.state0Height}}};b.select=function(){this.state0.visible=false;this.state1.visible=false;this.state2.visible=true};b.deselect=function(){this.state0.visible=false;this.state1.visible=true;this.state2.visible=false};b.minimize=function(){this.state0.visible=true;this.state1.visible=false;this.state2.visible=false};b._tick=function(){};argunet.SelectableView=a}());this.argunet=this.argunet||{};argunet.ArgunetBrowserView=function(g,d,a,f){this.canvasView=undefined;var c=d||640;var b=a||385;if(typeof(g)=="string"){g=$(g)}$(g).append("<div class='argunetBrowser loading' width='"+c+"' height='"+b+"'><canvas width='"+c+"' height='"+b+"'></canvas></div>");this.canvas=$(g).children(".argunetBrowser").children("canvas").get(0);this.stage=new createjs.Stage(this.canvas);this.stage.mouseEventsEnabled=true;this.stage.enableMouseOver(20);if(createjs.Touch.isSupported()){createjs.Touch.enable(this.stage)}this.navigationBar=new argunet.NavigationBarView($(this.canvas).parent());this.navigationOpened=false;this.debateListView=new argunet.DebateListView($(this.canvas).parent(),f);this.debateListView.setHeight($(this.canvas).parent().height()-this.navigationBar.height);var e=new createjs.Container();this.stage.addChild(e);this.tooltip=new argunet.TooltipCanvasView(this.canvas);this.stage.addChild(this.tooltip)};argunet.ArgunetBrowserView.prototype.handleEvent=function(a){var b=this;if(a.type=="showTooltip"&&!this.navigationOpened){this.tooltipTimeout=window.setTimeout(function(){b.tooltip.setVisible(true);b.tooltip.x=b.stage.mouseX+5;b.tooltip.y=b.stage.mouseY+5;b.tooltip.setContent(a.tooltip);b.stage.update();window.clearTimeout(this.tooltipTimeout)},1000)}else{if(a.type=="hideTooltip"&&!this.mouseOverTooltip){window.clearTimeout(this.tooltipTimeout);this.tooltip.setVisible(false);this.stage.update()}else{if(a.type=="mousedown"){if(!this.mouseOverTooltip){window.clearTimeout(this.tooltipTimeout);this.tooltip.setVisible(false);this.stage.update()}}else{if(a.type=="mouseover"&&a.target==this.tooltip){this.mouseOverTooltip=true}else{if(a.type=="mouseout"&&a.target==this.tooltip){this.mouseOverTooltip=false;this.tooltip.setVisible(false);this.stage.update()}else{if(a.type=="stagemousemove"){if(this.stage.mouseY>this.canvas.height-this.navigationBar.height){if(!this.navigationOpened){this.navigationBar.show();this.navigationOpened=true}}else{if(this.navigationOpened&&!this.debateListView.isOpen){this.navigationBar.hide();this.navigationOpened=false}}}}}}}}};argunet.ArgunetBrowserView.prototype.setCanvasView=function(a){var b=this;this.stage.clear();this.canvasView=a;this.canvasView.initialize(this.stage);this.tooltip.addEventListener("mouseover",this);this.tooltip.addEventListener("mouseout",this);this.stage.addEventListener("stagemousemove",this);this.navigationBar.addEventListener("openDebateList",function(){b.debateListView.show()});this.navigationBar.addEventListener("closeDebateList",function(){b.debateListView.hide()})};argunet.ArgunetBrowserView.prototype.removeLoadingSpinner=function(){$(this.canvas).parent().removeClass("loading");console.log("Canvas width: "+cWidth);if(typeof(htmlElement)=="string"){htmlElement=$(htmlElement)}$(htmlElement).append("<div class='argunetBrowser'><canvas width='"+cWidth+"' height='"+cHeight+"'></canvas></div>");this.canvas=$(htmlElement).children(".argunetBrowser").children("canvas").get(0);this.stage=new createjs.Stage(this.canvas);this.stage.mouseEventsEnabled=true;this.stage.enableMouseOver(20);if(createjs.Touch.isSupported()){createjs.Touch.enable(this.stage)}this.navigationBar=new argunet.NavigationBarView($(this.canvas).parent());this.navigationOpened=false;this.debateListView=new argunet.DebateListView($(this.canvas).parent(),browserId);this.debateListView.setHeight($(this.canvas).parent().height()-this.navigationBar.height);var a=new createjs.Container();this.stage.addChild(a);this.tooltip=new argunet.TooltipCanvasView(this.canvas);this.stage.addChild(this.tooltip)};argunet.ArgunetBrowserView.prototype.handleEvent=function(a){var b=this;if(a.type=="showTooltip"&&!this.navigationOpened){this.tooltipTimeout=window.setTimeout(function(){b.tooltip.setVisible(true);b.tooltip.x=b.stage.mouseX+5;b.tooltip.y=b.stage.mouseY+5;b.tooltip.setContent(a.tooltip);b.stage.update();window.clearTimeout(this.tooltipTimeout)},1000)}else{if(a.type=="hideTooltip"&&!this.mouseOverTooltip){window.clearTimeout(this.tooltipTimeout);
this.tooltip.setVisible(false);this.stage.update()}else{if(a.type=="mousedown"){if(!this.mouseOverTooltip){window.clearTimeout(this.tooltipTimeout);this.tooltip.setVisible(false);this.stage.update()}}else{if(a.type=="mouseover"&&a.target==this.tooltip){console.log("mouseover");this.mouseOverTooltip=true}else{if(a.type=="mouseout"&&a.target==this.tooltip){console.log("mouseout");this.mouseOverTooltip=false;this.tooltip.setVisible(false);this.stage.update()}else{if(a.type=="stagemousemove"){if(this.stage.mouseY>this.canvas.height-this.navigationBar.height){if(!this.navigationOpened){console.log("navi open");this.navigationBar.show();this.navigationOpened=true}}else{if(this.navigationOpened&&!this.debateListView.isOpen){console.log("navi closed");this.navigationBar.hide();this.navigationOpened=false}}}}}}}}};argunet.ArgunetBrowserView.prototype.setCanvasView=function(a){var b=this;this.stage.clear();this.canvasView=a;this.canvasView.initialize(this.stage);this.tooltip.addEventListener("mouseover",this);this.tooltip.addEventListener("mouseout",this);this.stage.addEventListener("stagemousemove",this);this.navigationBar.addEventListener("openDebateList",function(){b.debateListView.show()});this.navigationBar.addEventListener("closeDebateList",function(){b.debateListView.hide()})};this.argunet=this.argunet||{};(function(){var a=function(b,c,d){this.initialize(b,c,d)};a.prototype=new createjs.Container();a.prototype.Container_initialize=a.prototype.initialize;a.prototype.Container_tick=a.prototype._tick;a.prototype.initialize=function(b,c,d){this.Container_initialize();this.color=(b.type==undefined||b.type.toLowerCase()=="attack"||b.type.toLowerCase()=="sketchedattack")?"#ff0000":"#00ff00";this.edge=b;this.source=c;this.target=d};a.prototype.render=function(r,s){this.removeAllChildren();var m=18;var k=5;var q=5;var i=1;var h=Math.atan2(s.y-r.y,s.x-r.x);var f=h*(180/Math.PI);var p=s.x-r.x;var o=s.y-r.y;var n=Math.sqrt(p*p+o*o);n=n-m+q;var d=n*Math.cos(h);var u=n*Math.sin(h);d+=r.x;u+=r.y;var l=new createjs.Graphics();l.setStrokeStyle(i).beginStroke(this.color).moveTo(r.x,r.y).lineTo(d,u).endStroke();var j=new createjs.Shape(l);this.addChild(j);var t=new createjs.Graphics();t.beginFill(this.color).moveTo(-m,k).lineTo(0,0).lineTo(-m,-k).lineTo(-m+q,-0).closePath().endFill();var e=new createjs.Shape(t);e.x=s.x;e.y=s.y;e.rotation=f;this.addChild(e)};a.prototype._tick=function(){};argunet.ArrowCanvasView=a}());this.argunet=this.argunet||{};(function(){var a=function(d,c){this.initialize(d,c)};var b=a.prototype=new argunet.SelectableView();b.Selectable_initialize=a.prototype.initialize;b.Selectable_tick=a.prototype._tick;b.initialize=function(d,c){this.Selectable_initialize(d.id,d.title,d.description,c);this.argument=d;this.render()};argunet.ArgumentCanvasView=a}());this.argunet=this.argunet||{};(function(){var a=function(d,c){this.initialize(d,c)};var b=a.prototype=new argunet.SelectableView();b.Selectable_initialize=a.prototype.initialize;b.Selectable_tick=a.prototype._tick;b.initialize=function(d,c){this.Selectable_initialize(d.id,d.title,d.content,c);this.thesis=d;this.styleState1.borderColor=this.styleState2.borderColor=this.colors.base;this.styleState1.borderWidth=2;this.styleState2.borderWidth=4;this.styleState1.bgColor=this.styleState2.bgColor="#ffffff";this.styleState1.textColor=this.styleState2.textColor=this.styleState1.titleColor=this.styleState2.titleColor="#000000";this.marginState1.top=this.marginState1.right=this.marginState1.bottom=this.marginState1.left=1;this.marginState2.top=this.marginState2.right=this.marginState2.bottom=this.marginState2.left=2;this.render()};argunet.ThesisCanvasView=a}());this.argunet=this.argunet||{};argunet.ArborView=function(a){var k=a;var b=argunet.ArborView.prototype;b.addEventListener=null;b.removeEventListener=null;b.removeAllEventListeners=null;b.dispatchEvent=null;b.hasEventListener=null;b._listeners=null;createjs.EventDispatcher.initialize(b);var i=null;this.stage;var h={};var d={};var g=undefined;var c=undefined;var j=undefined;this.initialize=function(l){this.stage=l;j=new createjs.Container();this.stage.addChild(j);c=new createjs.Container();this.stage.addChild(c)};this.init=function(l){i=l;i.screenSize(this.stage.canvas.width,this.stage.canvas.height);i.screenPadding(40)};this.handleEvent=function(l){if(l.type=="select"){this.selectNode(l.target.nodeId);this.dispatchEvent({type:"nodeSelection",nodeId:l.target.nodeId},l.target)}else{if(l.type=="mouseover"){var q="<h3>"+l.target.title+"</h3><p>"+l.target.text+"</p>";if(l.target instanceof argunet.ArgumentCanvasView){q+="<table class='relations'><tr><td></td><td class='attacks'>Attacks</td><td class='supports'>Supports</td></tr><tr><td class='incoming'>Incoming</td><td class='attacks'>"+l.target.argument.relations.incomingAttacks+"</td><td class='supports'>"+l.target.argument.relations.incomingSupports+" </td></tr><tr><td class='outgoing'>Outgoing</td><td class='attacks'>"+l.target.argument.relations.outgoingAttacks+" </td><td class='supports'>"+l.target.argument.relations.outgoingSupports+" </td></tr></table>"}else{if(l.target instanceof argunet.ThesisCanvasView){q+="<table class='relations'><tr><td></td><td class='attacks'>Attacks</td><td class='supports'>Supports</td></tr><tr><td class='incoming'>Incoming</td><td class='attacks'>"+l.target.thesis.relations.incomingAttacks+"</td><td class='supports'>"+l.target.thesis.relations.incomingSupports+" </td></tr><tr><td class='outgoing'>Outgoing</td><td class='attacks'>"+l.target.thesis.relations.outgoingAttacks+" </td><td class='supports'>"+l.target.thesis.relations.outgoingSupports+" </td></tr></table>"}else{if(l.target instanceof argunet.GroupCanvasView){if(l.target.state2.visible){q+="<p class='tip'>(double click group to open the group and show its members.)</p>"}}}}this.dispatchEvent({type:"showTooltip",tooltip:q},l.target)}else{if(l.type=="mouseout"){this.dispatchEvent("hideTooltip",l.target)}else{if(l.type=="mousedown"){this.dispatchEvent({type:"mousedown",nodeId:l.target.nodeId},l.target)}else{if(l.type=="drag"){var n=l.target;var m=i.getNode(l.target.nodeId);m.fixed=true;var o={x:n.x,y:n.y};o=i.fromScreen(o);m.p=o}else{if(l.type=="drop"){var n=l.target;var m=i.getNode(l.target.nodeId);m.fixed=false;m.tempMass=50}else{if(l.type=="dblclick"){this.dispatchEvent({type:"openGroup",id:l.target.nodeId},l.target)}}}}}}}};this.selectNode=function(l){if(g!=undefined){g.deselect()}g=h[l];g.select()};this.redraw=function(){var m=this;if(!i){return}$.each(h,function(){var o=this.nodeId;if(i.getNode(o)==undefined){var n=h[o];if(g!=undefined&&n.nodeId==g.nodeId){g=undefined}n.removeEventListener(m);c.removeChild(n);delete h[o]}});$.each(d,function(){$.each(this,function(){if(i.getEdgesFrom(this.source)[this.target]==undefined){var n=d[this.source][this.target];j.removeChild(n);delete d[this.source][this.target]}})});var l={};i.eachNode(function(o,r){var q=h[o.data.id];if(h[o.data.id]==undefined){var s;if(o.data.group){s=k.groups[o.data.id]}else{s=k.getNode(o.data.id)}if(s instanceof argunet.Argument){q=new argunet.ArgumentCanvasView(s,k.getColor(s.colorIndex))}else{if(s instanceof argunet.Thesis){q=new argunet.ThesisCanvasView(s,k.getColor(s.colorIndex))}else{if(s instanceof argunet.Group){q=new argunet.GroupCanvasView(s);q.addEventListener("dblclick",m)}}}q.addEventListener("select",m);q.addEventListener("mousedown",m);q.addEventListener("mouseover",m);q.addEventListener("mouseout",m);q.addEventListener("drag",m);q.addEventListener("drop",m);h[o.data.id]=q;c.addChild(q)}var q=h[o.data.id];r.x=Math.floor(r.x);r.y=Math.floor(r.y);q.x=r.x;q.y=r.y;var n=q.getMarginedWidth();var p=q.getMarginedHeight();
l[o.data.id]=[r.x-n/2,r.y-p/2,n,p];if(o.data.edgesToSelectedNode>1){q.minimize()}else{if(o.data.edgesToSelectedNode>0){q.deselect()}}if(o.data.selected){m.selectNode(o.data.id)}});i.eachEdge(function(o,r,q){var n=l[o.data.target];var u={x:r.x,y:r.y};var t=f(u,q,n);var s=(d[o.data.source])?d[o.data.source][o.data.target]:undefined;if(s==undefined){if(d[o.data.source]==undefined){d[o.data.source]={}}var p=k.getEdge(o.data.sourceModel,o.data.targetModel);var s=new argunet.ArrowCanvasView(p,o.data.source,o.data.target);d[o.data.source][o.data.target]=s;j.addChild(s)}s.render(u,t)});this.stage.update()};var e=function(r,q,p,o){var l=((o.y-p.y)*(q.x-r.x)-(o.x-p.x)*(q.y-r.y));if(l===0){return false}var n=((o.x-p.x)*(r.y-p.y)-(o.y-p.y)*(r.x-p.x))/l;var m=((q.x-r.x)*(r.y-p.y)-(q.y-r.y)*(r.x-p.x))/l;if(n<0||n>1||m<0||m>1){return false}return arbor.Point(r.x+n*(q.x-r.x),r.y+n*(q.y-r.y))};var f=function(s,r,l){var p={x:l[0],y:l[1]},q=l[2],n=l[3];var u={x:p.x,y:p.y};var o={x:p.x+q,y:p.y};var m={x:p.x,y:p.y+n};var t={x:p.x+q,y:p.y+n};return e(s,r,u,o)||e(s,r,o,t)||e(s,r,t,m)||e(s,r,m,u)||false}};this.argunet=this.argunet||{};(function(){var a=function(c){this.initialize(c)};var b=a.prototype=new createjs.Container();b.Container_initialize=a.prototype.initialize;b.Container_tick=a.prototype._tick;b.initialize=function(c){this.Container_initialize();$(c).parent().append("<div class='argunet-tooltip'>Dies ist ein Tooltip</div>");this.tooltip=$(c).parent().children(".argunet-tooltip").get(0);var e=new createjs.DOMElement(this.tooltip);this.addChild(e);this.x=50;this.y=50;this.setVisible(false);var d=this;$(this.tooltip).mouseenter(function(f){d.dispatchEvent("mouseover",d)});$(this.tooltip).mouseleave(function(f){d.dispatchEvent("mouseout",d)})};b.setContent=function(c){$(this.tooltip).empty();$(this.tooltip).append(c)};b.setVisible=function(c){if(c){this.visible=true;$(this.tooltip).show()}else{this.visible=false;$(this.tooltip).hide()}};b._tick=function(){this.Container_tick()};argunet.TooltipCanvasView=a}());this.argunet=this.argunet||{};argunet.NavigationBarView=function(d){var c=argunet.NavigationBarView.prototype;c.addEventListener=null;c.removeEventListener=null;c.removeAllEventListeners=null;c.dispatchEvent=null;c.hasEventListener=null;c._listeners=null;createjs.EventDispatcher.initialize(c);var a="Open Debate List";d.append("<div class='navigationBar'><div class='buttons menuItem'><div class='back'>Back</div><div class='home'>Home</div><div class='forward'>Forward</div><div class='openList'>"+a+"</div></div><div class='graphDepth menuItem'><div class='label'>Graph Depth: <span class='graphDepthLabel'>1</span></div><div class='slider'></div></div></div>");this.htmlElement=$(d).children(".navigationBar").get(0);var b=this;$(this.htmlElement).find(".graphDepth .slider").slider({min:1,max:5,slide:function(e,f){$(b.htmlElement).find(".graphDepthLabel").text(f.value);b.dispatchEvent({type:"graphDepthChange",value:f.value},b)}});$(this.htmlElement).find(".buttons").buttonset();openListButton=$(this.htmlElement).find(".openList");openListButton.button().click(function(){var e=$(this).button("option","label");if(e==a){$(this).button("option","label","Close Debate List");b.dispatchEvent({type:"openDebateList"},b)}else{$(this).button("option","label",a);b.dispatchEvent({type:"closeDebateList"},b)}});$(this.htmlElement).find(".back").button({icons:{primary:"ui-icon-arrowthick-1-w"},text:false}).click(function(){b.dispatchEvent({type:"back"},b)});$(this.htmlElement).find(".home").button({icons:{primary:"ui-icon-home"},text:false}).click(function(){b.dispatchEvent({type:"home"},b)});$(this.htmlElement).find(".forward").button({icons:{primary:"ui-icon-arrowthick-1-e"},text:false}).click(function(){b.dispatchEvent({type:"forward"},b)});this.height=$(this.htmlElement).height();$(this.htmlElement).hide()};argunet.NavigationBarView.prototype.show=function(){console.log("fadeIn:");$(this.htmlElement).show("slide",{direction:"down"},200)};argunet.NavigationBarView.prototype.hide=function(){$(this.htmlElement).hide("slide",{direction:"down"},200)};argunet.NavigationBarView.prototype.setBackwardEnabled=function(a){$(this.htmlElement).find(".back").button({disabled:!a})};argunet.NavigationBarView.prototype.setForwardEnabled=function(a){$(this.htmlElement).find(".forward").button({disabled:!a})};argunet.NavigationBarView.prototype.setHomeEnabled=function(a){$(this.htmlElement).find(".home").button({disabled:!a})};this.argunet=this.argunet||{};argunet.DebateListView=function(d,c){var b=argunet.DebateListView.prototype;b.addEventListener=null;b.removeEventListener=null;b.removeAllEventListeners=null;b.dispatchEvent=null;b.hasEventListener=null;b._listeners=null;createjs.EventDispatcher.initialize(b);this.zTreeId=c+"-ztree";d.append("<div class='debateList'><div class='buttons'><div class='checkAll'>Show All Groups</div><div class='uncheckAll'>Unpack all Group Members</div></div><div id='"+this.zTreeId+"' class='tree ztree'></div></div>");this.htmlElement=$(d).children(".debateList").get(0);var a=this;$(this.htmlElement).hide();d.append("<div class='debateList'><div class='buttons'><div class='checkAll'>Show All Groups</div><div class='uncheckAll'>Unpack all Group Members</div></div><div id='"+this.zTreeId+"' class='tree ztree'></div></div>");this.htmlElement=$(d).children(".debateList").get(0);var a=this};argunet.DebateListView.prototype.initialize=function(g,b,f){if(b){$(this.htmlElement).prepend("<h4>"+b+"</h4>")}$(this.htmlElement).prepend("<h3>"+g+"</h3>");$(this.htmlElement).hide();var e=this;var c=function(i,k,j){if(j.nodeId!=undefined){e.dispatchEvent({type:"nodeSelection",nodeId:j.nodeId},e)}};var h=function(i,k,j){if(j.checked){e.dispatchEvent({type:"closeGroup",id:j.id},this)}else{e.dispatchEvent({type:"openGroup",id:j.id},this)}};var a={callback:{onClick:c,onCheck:h},check:{enable:true,chkboxType:{Y:"ps",N:"ps"}}};this.zTree=$.fn.zTree.init($("#"+this.zTreeId),a,f);var d=$.fn.zTree.getZTreeObj(this.zTreeId);$(this.htmlElement).children(".buttons").buttonset();$(this.htmlElement).find(".checkAll").button().click(function(){d.checkAllNodes(true);e.dispatchEvent({type:"closeAllGroups"},e)});$(this.htmlElement).find(".uncheckAll").button().click(function(){d.checkAllNodes(false);e.dispatchEvent({type:"openAllGroups"},e)})};argunet.DebateListView.prototype.setHeight=function(a){this.height=a;$(this.htmlElement).height(this.height)};argunet.DebateListView.prototype.show=function(){$(this.htmlElement).show("slide",{direction:"left"},600);this.isOpen=true};argunet.DebateListView.prototype.hide=function(){$(this.htmlElement).hide("slide",{direction:"left"},600);this.isOpen=false};this.argunet=this.argunet||{};(function(){var b=function(c){this.initialize(c)};var a=b.prototype=new argunet.SelectableView();a.Selectable_initialize=b.prototype.initialize;a.Selectable_tick=b.prototype._tick;a.initialize=function(d){var c=d.numberOfArguments+d.numberOfTheses;this.Selectable_initialize(d.id,"Group: "+d.title,c+" Members",{base:d.color,dark:"#666",light:d.color});this.group=d;this.styleState2.textCss="bold 18px Arial";this.styleState2.borderWidth=1;this.styleState1.borderWidth=1;this.styleState1.cornerRadius=20;this.styleState2.cornerRadius=35;this.styleState0.cornerRadius=10;this.state0Height=25;this.paddingState1={top:12,left:12,bottom:12,right:12};this.paddingState2={top:20,left:20,bottom:20,right:20};this.render()};argunet.GroupCanvasView=b}());this.argunet=this.argunet||{};argunet.ErrorMessageView=function(f,d,a,g,e){this.canvasView=undefined;var c=d||640;var b=a||385;if(typeof(f)=="string"){f=$(f)}$(f).append("<div class='argunetBrowser loading' width='"+c+"' height='"+b+"'><h3>"+g+"</h3><p>"+e+"</p></div>")};this.argunet=this.argunet||{};argunet.ArgunetBrowser=function(d,h,g,c,a){var f=argunet.ArgunetBrowser.prototype;f.addEventListener=null;f.removeEventListener=null;f.removeAllEventListeners=null;f.dispatchEvent=null;f.hasEventListener=null;f._listeners=null;createjs.EventDispatcher.initialize(f);this.browserId=argunet.BrowserRegistry.getInstance().registerBrowser(this);if(!Modernizr.canvas||!Modernizr.canvastext){new argunet.ErrorMessageView(h,c,a,"Argunet Browser not initialized","We detected that your browser lacks features <a href='http://www.argunet.org'>Argunet Browser</a> depends on. Please use an up-to-date browser that supports HTML5, CSS3 and the Canvas Element.");return}this.argunetView=new argunet.ArgunetBrowserView(h,c,a,this.browserId);var d=d;var b=g;var e=this;$.ajax({type:"GET",url:d,dataType:"xml",success:function(i){xml=$(i);e.onDebateLoad(xml)},error:function(i,k,j){alert("Data Could Not Be Loaded - "+k)}});this.onDebateLoad=function(i){this.debateManager=new argunet.DebateManager();this.debateManager.loadDebate(i);this.debateListView=this.argunetView.debateListView;this.debateListController=new argunet.DebateListController(this.debateListView,this.debateManager);this.arborView=new argunet.ArborView(this.debateManager);this.argunetView.setCanvasView(this.arborView);this.arborController=new argunet.ArborController(this.arborView,this.debateManager);this.debateListView.addEventListener("nodeSelection",this.arborController);this.history=new argunet.History();this.arborView.addEventListener("mousedown",this.argunetView);this.argunetView.navigationBar.addEventListener("back",this.history);this.argunetView.navigationBar.addEventListener("home",this.history);this.argunetView.navigationBar.addEventListener("forward",this.history);this.argunetView.navigationBar.addEventListener("graphDepthChange",this.arborController);this.arborView.addEventListener("showTooltip",this.argunetView);this.arborView.addEventListener("hideTooltip",this.argunetView);this.arborView.addEventListener("mousedown",this.argunetView);this.history.addEventListener("historyChange",this);this.history.addEventListener("historyChange",this.arborController);this.debateListView.addEventListener("nodeSelection",this);this.debateListView.addEventListener("openGroup",this.arborController);this.arborView.addEventListener("openGroup",this.arborController);this.debateListView.addEventListener("closeGroup",this.arborController);this.debateListView.addEventListener("openAllGroups",this.arborController);this.debateListView.addEventListener("closeAllGroups",this.arborController);this.arborView.addEventListener("nodeSelection",this);this.argunetView.removeLoadingSpinner();if(b==undefined){$.each(this.debateManager.nodes,function(){b=this.id;return false})}this.history.selectNode(b)};var d=d;var b=g;var e=this;this.handleEvent=function(i){if(i.type=="historyChange"){this.argunetView.navigationBar.setBackwardEnabled(this.history.backwardEnabled);this.argunetView.navigationBar.setForwardEnabled(this.history.forwardEnabled);this.argunetView.navigationBar.setHomeEnabled(this.history.homeEnabled)}else{if(i.type=="nodeSelection"){this.history.selectNode(i.nodeId);this.dispatchEvent({type:"nodeSelection",nodeId:i.nodeId},i.target)}}}};